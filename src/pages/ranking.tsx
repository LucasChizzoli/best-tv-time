...
const Ranking: NextPage = () => {
  ...
  const getVotesPercent = (dog: {
    name: string;
    imageUrl: string;
    _count: { VoteFor: number; VoteAgainst: number };
  }) => {
  ...
  return (
    <div className="flex flex-col py-9 items-center bg-gray-800 text-white">
      <h1 className="text-4xl my-5">Cutest Top 10 dogs!</h1>
      <div>
        {rankings &&
          rankings.map((ranking) => (
            <div
              key={ranking.id}
              className="flex border-b border-gray-500 gap-4 my-3 py-3"
            >
              <div>
                <Image
                  src={
              ...
              <div>
                ...
                <p className="mt-3">
                  Cutets: {getVotesPercent(ranking).toFixed(2)}%
                </p>
              </div>
            </div>
          ))}
